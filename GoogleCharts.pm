...
sub draw_piechart {
    ...
    my @data_keys = sort keys %$data_href;
    ...
    $text .= " style=\"width: ${width}px; height: ${height}px;\"></div>\n";
...
####
sub draw_scatter_plot {
    my %chart_params = @_;

    
    my $data_aref = $chart_params{data} or die "Error, need data"; # format  [ ["Xname", "Yname"], [x1,y1], [x2,y2], ... ] 
    
    my $x_axis_name = $chart_params{x_axis_name} || "X-axis";
    my $y_axis_name = $chart_params{y_axis_name} || "Y-axis";
    ...
    for (my $i = 0; $i <= $#$data_aref; $i++) {
        $text .= " [$data_aref->[$i]->[0], $data_aref->[$i]->[1]]";
        ...
        if ($i == 0) {
            ...
            if ($data_aref->[$i]->[0] > $max_i) {
                $max_i = $data_aref->[$i]->[0];
            }
            if ($data_aref->[$i]->[1] > $max_j) {
                $max_j = $data_aref->[$i]->[1];
            }
            if ($data_aref->[$i]->[0] < $min_i) {
                $min_i = $data_aref->[$i]->[0];
            }
            if ($data_aref->[$i]->[1] < $min_j) {
                $min_j = $data_aref->[$i]->[1];
            }
    ...
    $text .= "    var options = {\n";
    $text .= "    title: \'$chart_title\',\n";
    $text .= "    hAxis: {title: \'$x_axis_name\', minValue: 0, maxValue: $max_i},\n";
    $text .= "    vAxis: {title: \'$y_axis_name\', minValue: 0, maxValue: $max_j},\n";
    $text .= "    legend: 'none'\n";
    $text .= "};\n";

    $text .= "    var chart = new google.visualization.ScatterChart(document.getElementById(\'$div_id\'));\n";
    $text .= "    chart.draw(data, options);\n";
    $text .= "}\n";
    $text .= "</script>\n";
 
    $text .= "<div id=\"$div_id\" style=\"width: 900px; height: 500px;\"></div>\n";
